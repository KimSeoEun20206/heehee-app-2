...
#1. 텍스트 작성하기
st.title("heehee╰(*°▽°*)╯") #제목 붙이기
st.write("내가 다 먹을께") #제목 밑 글자 표시(자유)


#2. 웹에 게시된 csv파일 불러오기
url = "https://github.com/teacher188-netizen/blank-app/blob/main/meals_data.csv"+"?raw=true" #주소 너무 기니까 url로 지정함
df = pd.read_csv( url, encoding = 'cp949') #pandas에서 제공하는 주소를 읽는 도구를 이용
st.dataframe(df) #스트리밋에 데이터 보여주기,알아서 정렬해줌 dataframe이

#3. 데이터 대시보드 만들기
st.write("오늘의 메뉴가 뭔지 봐")

#오늘 날짜 불러오기
dt = str(date.today()) #오늘 날짜가 문자로 변환되어서 dt로 저장
st.write(dt)
today_row =df.loc[df['급식일자']=='2025-09-08'] #조건을 통해 행 불러오기(오늘 날짜와 일치하는 급식 나타나기)
st.write(today_row) #오늘 날짜에 해당하는 행 출력

#metric 활용하기
st.write("metric으로 통계 정보를 전광판 형태로 시각화할 수 있어요.")
st.title(today_row['요리명'].item())
st.metric("Menu", today_row['요리명'].item(),border=True)

#metric 열 구성하기
a,b=st.columns(2) #열 만들기
#a, b에 metric 만들기
...
map_data = pd.DataFrame({
    ...
    'students': [923, 662, 1109] #유동인구에 따라 크기가 다름
})
...
with st.form("heehee급식 의견 말해주세요"):
    a=st.date_input("날짜를 선택할 수 있는 입력폼")
    ...
    e=st.radio("객관식 버튼 입력폼", ["1", "2", "3", "4", "5"])
